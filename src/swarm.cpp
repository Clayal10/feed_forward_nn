...
double even_rand_double(auto dis, auto gen){
	return dis(gen);
}
...
Swarm::particle::particle(){
	...
	for(int i=0; i<weight_amt; i++){
		weights.push_back(even_rand_double(dis, gen));
		velocity.push_back(even_rand_double(dis, gen));
	}
	...
	//for(int i=0; i<4; i++){
		...
		if(i != 2 && i!= 3){
			...
			for(int k=0; k<network[1]; k++){
				for(int l=0; l<network[1]; l++){
					buffer.push_back(weights[j]);
					//std::cout << weights[j] << std::endl;
					j++;
				}
...
void Swarm::iterate_swarm(){ // at this point, the swarm is initialized
	...
	while(count < iterations){
		/*I would like to multithread this up to 30 (maybe) threads.*/
